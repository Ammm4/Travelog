...
import { PostForumWrapper, PostBody,PostTitle, StyledParagraph } from '../GlobalComponents/StyledComponents/Containers';
...
import { PostImg } from '../GlobalComponents/StyledComponents/Images';
import PostSkeleton from '../Skeleton.js/PostSkeleton';
import CommentsAndLikes from '../GlobalComponents/Components/CommentsAndLikes';
import { FaRegComment, FaRegHeart, FaHeart } from "react-icons/fa";
import { likePost, setPostsPostData, setPostData } from '../../../../redux/posts/postActions';
...
export default function Post({ post, postMarkerRef }) {
  ...
  const handleCommentBtn = () => {
    if(singlePost) return dispatch(setPostData('showComments', true))
    dispatch(setPostsPostData(_id,'showComments',true))
  }
  if(post.deleted) {
    return <PostSkeleton deleted={true} postMarkerRef={postMarkerRef}/>
  }
  return (
        <PostForumWrapper singlePost={ singlePost } ref={ postMarkerRef }>   
          ...
          <PostBody>
            ...
            <h4>Images ({ images.length })</h4>
             <PostForumLink to={`/dashboard/posts/${_id}`} onClick={() => handleLink(_id)}>
